...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector('.book-now-btn')) {
        ...
        if (bookBtn.hasAttribute('disabled')) {
            bookBtn.removeAttribute('disabled');
        }
    }
    
    // На странице index25.html (оплата)
    if (document.querySelector('.complete-booking-btn')) {
        document.querySelector('.complete-booking-btn').addEventListener('click', function() {
            window.location.href = 'index35.html';
        });
    }
    
    // На странице index35.html (подтверждение)
    if (document.querySelector('.final-btn-back')) {
        document.querySelector('.final-btn-back').addEventListener('click', function(e) {
            e.preventDefault();
